...
fn do_rounds(monkeys: &mut HashMap<u128, Monkey>, n: u128, correction: u128) -> u128 {
    ...
    for i in 1 .. n + 1 {
        ...
        for monkey_id in 0 .. m {
            ...
            let items_to = monkey.inspect_items(&correction);
...
impl Monkey {
    ...
    fn inspect_item(&self, level: &u128, correction: &u128) -> [u128; 2] {
    ...
    fn inspect_items(&mut self, correction: &u128) -> Vec<[u128; 2]> {
        let mut items_to: Vec<[u128; 2]> = vec![];
        for item in &self.items {
            items_to.push(self.inspect_item(&item, correction));
            self.inspected += 1;
        }
        self.items.clear();
        items_to
